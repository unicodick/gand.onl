...
<div class="flex items-start justify-between gap-4">
  <div class="space-y-2">
    <a
      ...
      class="inline-flex items-center gap-2 text-[10px] tracking-widest text-neutral-400 hover:text-neutral-200"
    >
...
>
  ...
  >
    ...
    <input
      ...
      disabled
      class="border border-white/5 bg-black/10 px-3 py-3 text-[10px] text-neutral-400"
    />
  </label>
  ...
  >
    ...
    <input
      ...
      class="border border-white/10 bg-black/20 px-3 py-3 text-[10px] text-neutral-100 outline-none placeholder:text-neutral-400 focus:border-grass-dim"
    />
  </label>
  ...
  <div class="flex justify-end border-t border-white/5 pt-5">
    <button
      type="submit"
      class="mc-tab inline-flex items-center gap-2 px-4 py-3 text-[10px] tracking-widest text-neutral-200 hover:text-white"
    >
...
<section class="mc-panel space-y-4 p-5 sm:p-6">
  ...
  {#if data.player.blocked_at}
    <div class="space-y-2 text-[10px] leading-relaxed">
      ...
      {#if data.player.block_reason}
        <p class="text-neutral-400">{data.player.block_reason}</p>
      {/if}
    </div>
    <form method="POST" action="?/unblock">
      <button
        type="submit"
        class="mc-tab inline-flex items-center gap-2 px-4 py-3 text-[10px] tracking-widest text-grass hover:text-white"
      >
  ...
  {:else}
    <form method="POST" action="?/block" class="space-y-3">
      ...
      <button
        type="submit"
        class="mc-tab inline-flex items-center gap-2 px-4 py-3 text-[10px] tracking-widest text-red-400 hover:text-red-300"
      >
...
<section class="space-y-4 border border-red-500/20 p-5 sm:p-6">
  ...
  <form method="POST" action="?/delete" class="flex flex-col gap-3 sm:flex-row">
    <input
      ...
      autocomplete="off"
      class="min-w-0 flex-1 border border-white/10 bg-black/20 px-3 py-3 text-[10px] text-neutral-100 outline-none placeholder:text-neutral-400 focus:border-red-500/60"
    />
    <button
      type="submit"
      disabled={deleteConfirmation !== data.player.username}
      class="mc-tab inline-flex items-center justify-center gap-2 px-4 py-3 text-[10px] tracking-widest text-red-400 hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-30"
    >
